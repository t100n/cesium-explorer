...
box2D.collision.B2AABB.prototype = {
    ...
    rayCast: function (output, input) {
        var tmin = -1.7976931348623157e308;
        var tmax = 1.7976931348623157e308;
        var pX = input.p1.x;
        var pY = input.p1.y;
...
box2D.common.math.B2Vec2.prototype = {
    ...
    minV: function (b) {
        if (this.x < b.x) this.x = this.x;
        else this.x = b.x; if (this.y < b.y) this.y = this.y;
        else this.y = b.y
    },
    maxV: function (b) {
        if (this.x > b.x) this.x = this.x;
    ...
    normalize: function () {
        ...
        if (length < 2.2250738585072014e-308) return 0;
...
box2D.collision.B2Collision.edgeSeparation = function (poly1, xf1, edge1, poly2, xf2) {
    ...
    var index = 0;
    var minDot = 1.7976931348623157e308;
    var _g = 0;
    while (_g < count2) {
        var i = _g++;
        tVec = vertices2[i];
...
box2D.collision.B2Collision.findMaxSeparation = function (edgeIndex, poly1, xf1, poly2, xf2) {
    ...
    var edge = 0;
    var maxDot = -1.7976931348623157e308;
    var _g = 0;
    while (_g < count1) {
        var i = _g++;
        tVec = normals1[i];
    ...
    var prevEdge;
    if (edge - 1 >= 0) prevEdge = edge - 1;
    else prevEdge = count1 - 1;
    var sPrev = box2D.collision.B2Collision.edgeSeparation(poly1, xf1, prevEdge, poly2, xf2);
    var nextEdge;
    if (edge + 1 < count1) nextEdge = edge + 1;
    else nextEdge = 0;
    ...
    while (true) {
        if (increment == -1)
            if (bestEdge - 1 >= 0) edge = bestEdge - 1;
            else edge = count1 - 1;
        else if (bestEdge + 1 < count1) edge = bestEdge + 1;
        else edge = 0;
...
box2D.collision.B2Collision.collidePolygons = function (manifold, polyA, xfA, polyB, xfB) {
    ...
    var local_v12;
    if (edge1 + 1 < count1) local_v12 = vertices1[edge1 + 1 | 0];
...
box2D.collision.B2Collision.collidePolygonAndCircle = function (manifold, polygon, xf1, circle, xf2) {
    ...
    var dist;
    var normalIndex = 0;
    var separation = -1.7976931348623157e308;
    ...
    var vertIndex2;
    if (vertIndex1 + 1 < vertexCount) vertIndex2 = vertIndex1 + 1;
    else vertIndex2 = 0;
...
box2D.collision.B2ContactPoint.__name__ = true;
...
box2D.collision.B2Simplex.prototype = {
    readCache: function (cache, proxyA, transformA, proxyB, transformB) {
        ...
        if (this.m_count > 1) {
            ...
            if (metric2 < .5 * metric1 || 2 * metric1 < metric2 || metric2 < 2.2250738585072014e-308) this.m_count = 0
        }
        if (this.m_count == 0) {
            v = vertices[0];
            v.indexA = 0;
    ...
    getSearchDirection: function () {
        ...
        switch (_g) {
            ...
            case 2:
                ...
                if (sgn > 0) return box2D.common.math.B2Math.crossFV(1, e12);
                else return box2D.common.math.B2Math.crossVF(e12, 1);
                break;
            default:
...
box2D.collision.B2Distance.distance = function (output, cache, input) {
    ...
    while (iter < k_maxIters) {
        ...
        if (d.lengthSquared() < 0) break;
        var vertex = vertices[simplex.m_count];
        var x = proxyA.getSupport(box2D.common.math.B2Math.mulTMV(transformA.R, d.getNegative()));
        vertex.indexA = x | 0;
        vertex.wA = box2D.common.math.B2Math.mulX(transformA, proxyA.getVertex(vertex.indexA));
        var x = proxyB.getSupport(box2D.common.math.B2Math.mulTMV(transformB.R, d));
        vertex.indexB = x | 0;
    ...
    var x = box2D.common.math.B2Math.max(box2D.collision.B2Distance.b2_gjkMaxIters, iter);
    box2D.collision.B2Distance.b2_gjkMaxIters = x | 0;
    ...
    if (input.useRadii) {
        ...
        if (output.distance > rA + rB && output.distance > 2.2250738585072014e-308) {
...
box2D.collision.B2DynamicTree.prototype = {
    ...
    rebalance: function (iterations) {
        ...
        while (_g < iterations) {
            ...
            while (node.isLeaf() == false) {
                if ((this.m_path >> bit & 1) != 0) node = node.child2;
    ...
    rayCast: function (callbackMethod, input) {
        ...
        r.normalize();
        var v = box2D.common.math.B2Math.crossFV(1, r);
    ...
    removeLeaf: function (leaf) {
        ...
        else sibling = node2.child1; if (node1 != null) {
...
box2D.collision.B2DynamicTreeBroadPhase.prototype = {
    ...
    updatePairs: function (callbackMethod) {
        ...
        var i = 0;
        while (pairing)
            if (i >= this.m_pairCount) pairing = false;
            else {
...
box2D.collision.B2OBB.__name__ = true;
...
box2D.collision.B2RayCastInput.__name__ = true;
...
box2D.collision.B2RayCastOutput.__name__ = true;
...
box2D.collision.B2SeparationFunction.prototype = {
    initialize: function (cache, proxyA, transformA, proxyB, transformB) {
        ...
        if (count == 1) {
            ...
            this.m_axis = box2D.common.math.B2Math.crossVF(box2D.common.math.B2Math.subtractVV(localPointA2, localPointA1), 1);
            this.m_axis.normalize();
            tVec = this.m_axis;
            tMat = transformA.R;
            ...
            this.m_axis = box2D.common.math.B2Math.crossVF(box2D.common.math.B2Math.subtractVV(localPointB2, localPointB1), 1);
            this.m_axis.normalize();
            tVec = this.m_axis;
            tMat = transformB.R;
            normalX = tMat.col1.x * tVec.x + tMat.col2.x * tVec.y;
            ...
            var denom = a * e - b * b;
            s = 0;
            if (denom != 0) s = box2D.common.math.B2Math.clamp((b * f - c * e) / denom, 0, 1);
            var t = (b * s + f) / e;
            if (t < 0) {
                t = 0;
                s = box2D.common.math.B2Math.clamp((b - c) / a, 0, 1)
            }
    ...
    evaluate: function (transformA, transformB) {
        ...
        var seperation;
        var normal;
        if (this.m_type == box2D.collision.B2SeparationFunction.e_points) {
            ...
            return seperation
        } else if (this.m_type == box2D.collision.B2SeparationFunction.e_faceA) {
            ...
            return seperation
        } else if (this.m_type == box2D.collision.B2SeparationFunction.e_faceB) {
...
box2D.common.math.B2Mat22 = function () {
    this.col1 = new box2D.common.math.B2Vec2(0, 1);
    this.col2 = new box2D.common.math.B2Vec2(0, 1)
};
...
box2D.collision.B2TimeOfImpact.timeOfImpact = function (input) {
    ...
    box2D.common.B2Settings.b2Assert(1 - sweepA.t0 > 2.2250738585072014e-308);
    ...
    while (true) {
        ...
        if (box2D.collision.B2TimeOfImpact.s_distanceOutput.distance <= 0) {
            alpha = 1;
            break
        }
        ...
        if (iter == 0) {
            if (separation > radius) target = box2D.common.math.B2Math.max(radius - tolerance, .75 * radius);
            else target = box2D.common.math.B2Math.max(separation - tolerance, .02 * radius)
        }
        ...
        while (true) {
            var x;
            if ((rootIterCount & 1) != 0) x = x1 + (target - f1) * (x2 - x1) / (f2 - f1);
            else x = .5 * (x1 + x2);
            ...
            if (box2D.common.math.B2Math.abs(f - target) < .025 * tolerance) {
                newAlpha = x;
                break
            }
            if (f > target) {
        ...
        var x = box2D.common.math.B2Math.max(box2D.collision.B2TimeOfImpact.b2_toiMaxRootIters, rootIterCount);
        box2D.collision.B2TimeOfImpact.b2_toiMaxRootIters = x | 0;
    ...
    var x = box2D.common.math.B2Math.max(box2D.collision.B2TimeOfImpact.b2_toiMaxIters, iter);
    box2D.collision.B2TimeOfImpact.b2_toiMaxIters = x | 0;
    return alpha
};
        ...
        switch (_g) {
            ...
            case box2D.collision.B2Manifold.e_faceA:
                ...
                while (_g2 < _g1) {
                    ...
                    this.m_points[i1].x = clipPointX + .5 * (radiusA - (clipPointX - planePointX) * normalX - (clipPointY - planePointY) * normalY - radiusB) * normalX;
                    this.m_points[i1].y = clipPointY + .5 * (radiusA - (clipPointX - planePointX) * normalX - (clipPointY - planePointY) * normalY - radiusB) * normalY
                }
                break;
            case box2D.collision.B2Manifold.e_faceB:
                ...
                while (_g2 < _g1) {
                    ...
                    this.m_points[i1].x = clipPointX + .5 * (radiusB - (clipPointX - planePointX) * normalX - (clipPointY - planePointY) * normalY - radiusA) * normalX;
                    this.m_points[i1].y = clipPointY + .5 * (radiusB - (clipPointX - planePointX) * normalX - (clipPointY - planePointY) * normalY - radiusA) * normalY
                }
                break
        }
    },
...
box2D.collision.shapes.B2CircleShape.prototype = $extend(box2D.collision.shapes.B2Shape.prototype, {
    ...
    rayCast: function (output, input, transform) {
        ...
        var rr = rX * rX + rY * rY;
        var sigma = c * c - rr * b;
        if (sigma < 0 || rr < 2.2250738585072014e-308) return false;
    ...
    computeSubmergedArea: function (normal, offset, xf, c) {
        ...
        if (l < -this.m_radius + 2.2250738585072014e-308) return 0;
        if (l > this.m_radius) {
            c.setV(p);
        ...
        var com = -.6666666666666666 * Math.pow(r2 - l2, 1.5) / area;
...
box2D.collision.shapes.B2EdgeShape.prototype = $extend(box2D.collision.shapes.B2Shape.prototype, {
    ...
    rayCast: function (output, input, transform) {
        ...
        var k_slop = 2.2250738585072014e-306;
...
box2D.collision.shapes.B2PolygonShape.computeCentroid = function (vs, count) {
    ...
    var p1X = 0;
    var p1Y = 0;
    var inv3 = .3333333333333333;
    var _g = 0;
    while (_g < count) {
        var i = _g++;
        var p2 = vs[i];
        var p3;
        if (i + 1 < count) p3 = vs[i + 1 | 0];
        else p3 = vs[0];
        var e1X = p2.x - p1X;
        var e1Y = p2.y - p1Y;
        var e2X = p3.x - p1X;
...
box2D.collision.shapes.B2PolygonShape.computeOBB = function (obb, vs, count) {
    ...
    p[count] = p[0];
    var minArea = 1.7976931348623157e308;
    var _g1 = 1;
    var _g = count + 1;
    while (_g1 < _g) {
        var i1 = _g1++;
        ...
        var uyX = -uxY;
        var uyY = uxX;
        var lowerX = 1.7976931348623157e308;
        var lowerY = 1.7976931348623157e308;
        var upperX = -1.7976931348623157e308;
        var upperY = -1.7976931348623157e308;
        var _g2 = 0;
        while (_g2 < count) {
            var j = _g2++;
            var dX = p[j].x - root.x;
...
box2D.collision.shapes.B2PolygonShape.prototype = $extend(box2D.collision.shapes.B2Shape.prototype, {
    ...
    setAsVector: function (vertices, vertexCount) {
        ...
        while (_g1 < _g) {
            ...
            var i11 = i1;
            var i2;
            if (i1 + 1 < this.m_vertexCount) i2 = i1 + 1;
            else i2 = 0;
            var edge = box2D.common.math.B2Math.subtractVV(this.m_vertices[i2], this.m_vertices[i11]);
            box2D.common.B2Settings.b2Assert(edge.lengthSquared() > 2.2250738585072014e-308);
            this.m_normals[i1].setV(box2D.common.math.B2Math.crossVF(edge, 1));
    ...
    setAsEdge: function (v1, v2) {
        ...
        this.m_normals[0] = box2D.common.math.B2Math.crossVF(box2D.common.math.B2Math.subtractVV(v2, v1), 1);
    ...
    rayCast: function (output, input, transform) {
        ...
        while (_g1 < _g) {
            ...
            if (upper < lower - 2.2250738585072014e-308) return false
        }
        if (index >= 0) {
    ...
    computeAABB: function (aabb, xf) {
        ...
        while (_g1 < _g) {
            ...
            if (lowerX < vX) lowerX = lowerX;
            else lowerX = vX; if (lowerY < vY) lowerY = lowerY;
            else lowerY = vY; if (upperX > vX) upperX = upperX;
            else upperX = vX; if (upperY > vY) upperY = upperY;
            else upperY = vY
        }
    ...
    computeMass: function (massData, density) {
        ...
        while (_g1 < _g) {
            ...
            var p3;
            if (i + 1 < this.m_vertexCount) p3 = this.m_vertices[i + 1 | 0];
            else p3 = this.m_vertices[0];
            var e1X = p2.x - p1X;
            var e1Y = p2.y - p1Y;
            var e2X = p3.x - p1X;
            var e2Y = p3.y - p1Y;
    ...
    computeSubmergedArea: function (normal, offset, xf, c) {
        ...
        while (_g1 < _g) {
            ...
            var isSubmerged = depths[i1] < -2.2250738585072014e-308;
            if (i1 > 0) {
                if (isSubmerged) {
                    if (!lastSubmerged) {
                        intoIndex = i1 - 1;
...
box2D.common.B2Color = function (rr, gg, bb) {
    var x = 255 * box2D.common.math.B2Math.clamp(rr, 0, 1);
    this._r = x | 0;
    var x = 255 * box2D.common.math.B2Math.clamp(gg, 0, 1);
    this._g = x | 0;
    var x = 255 * box2D.common.math.B2Math.clamp(bb, 0, 1);
    this._b = x | 0
};
...
box2D.common.B2Settings.b2MixRestitution = function (restitution1, restitution2) {
    if (restitution1 > restitution2) return restitution1;
...
box2D.common.math.B2Mat33.__name__ = true;
...
box2D.common.math.B2Sweep.prototype = {
    ...
    advance: function (t) {
        if (this.t0 < t && 1 - this.t0 > 2.2250738585072014e-308) {
...
box2D.common.math.B2Vec3.__name__ = true;
...
box2D.dynamics.B2Body.prototype = {
    connectEdges: function (s1, s2, angle1) {
        ...
        cornerDir.normalize();
        var convex = box2D.common.math.B2Math.dot(s1.getDirectionVector(), s2.getNormalVector()) > 0;
    ...
    setMassData: function (massData) {
        ...
        if (massData.I > 0 && (this.m_flags & box2D.dynamics.B2Body.e_fixedRotationFlag) == 0) {
    ...
    resetMassData: function () {
        ...
        if (this.m_mass > 0) {
            ...
            this.m_mass = 1;
            this.m_invMass = 1
        } if (this.m_I > 0 && (this.m_flags & box2D.dynamics.B2Body.e_fixedRotationFlag) == 0) {
    ...
    setBullet: function (flag) {
        if (flag) this.m_flags |= box2D.dynamics.B2Body.e_bulletFlag;
        else this.m_flags &= ~box2D.dynamics.B2Body.e_bulletFlag
    },
    isBullet: function () {
    ...
    setSleepingAllowed: function (flag) {
        if (flag) this.m_flags |= box2D.dynamics.B2Body.e_allowSleepFlag;
        else {
    ...
    setFixedRotation: function (fixed) {
        if (fixed) this.m_flags |= box2D.dynamics.B2Body.e_fixedRotationFlag;
        else this.m_flags &= ~box2D.dynamics.B2Body.e_fixedRotationFlag;
        this.resetMassData()
    },
...
box2D.dynamics.B2Island.prototype = {
    ...
    solve: function (step, gravity, allowSleep) {
        ...
        while (_g1 < _g) {
            ...
            b.m_linearVelocity.multiply(box2D.common.math.B2Math.clamp(1 - step.dt * b.m_linearDamping, 0, 1));
            b.m_angularVelocity *= box2D.common.math.B2Math.clamp(1 - step.dt * b.m_angularDamping, 0, 1)
        }
        ...
        var _g1 = 0;
        var _g = step.velocityIterations;
        while (_g1 < _g) {
            var i1 = _g1++;
            var _g3 = 0;
        ...
        while (_g1 < _g) {
            ...
            if (rotation * rotation > box2D.common.B2Settings.b2_maxRotationSquared) {
                if (b.m_angularVelocity < 0) b.m_angularVelocity = -box2D.common.B2Settings.b2_maxRotation * step.inv_dt;
                else b.m_angularVelocity = box2D.common.B2Settings.b2_maxRotation * step.inv_dt
            }
        ...
        var _g1 = 0;
        var _g = step.positionIterations;
        while (_g1 < _g) {
            var i1 = _g1++;
        ...
        if (allowSleep) {
            var minSleepTime = 1.7976931348623157e308;
    ...
    solveTOI: function (subStep) {
        ...
        var _g1 = 0;
        var _g = subStep.velocityIterations;
        while (_g1 < _g) {
            var i1 = _g1++;
            contactSolver.solveVelocityConstraints();
            var _g3 = 0;
        ...
        while (_g1 < _g) {
            ...
            if (rotation * rotation > box2D.common.B2Settings.b2_maxRotationSquared) {
                if (b.m_angularVelocity < 0) b.m_angularVelocity = -box2D.common.B2Settings.b2_maxRotation * subStep.inv_dt;
                else b.m_angularVelocity = box2D.common.B2Settings.b2_maxRotation * subStep.inv_dt
            }
            b.m_sweep.c0.setV(b.m_sweep.c);
            b.m_sweep.a0 = b.m_sweep.a;
        ...
        var _g1 = 0;
        var _g = subStep.positionIterations;
        while (_g1 < _g) {
            var i1 = _g1++;
...
box2D.dynamics.B2World.prototype = {
    ...
    step: function (dt, velocityIterations, positionIterations) {
        ...
        if (box2D.dynamics.B2World.m_continuousPhysics && step.dt > 0) this.solveTOI(step);
    ...
    rayCast: function (callbackMethod, point1, point2) {
        ...
        var rayCastWrapper = function (input, proxy) {
            ...
            if (hit) {
                ...
                var point = new box2D.common.math.B2Vec2((1 - fraction) * point1.x + fraction * point2.x, (1 - fraction) * point1.y + fraction * point2.y);
    ...
    solveTOI: function (step) {
        ...
        while (true) {
            ...
            while (c != null) {
                ...
                var toi = 1;
                if ((c.m_flags & box2D.dynamics.contacts.B2Contact.e_toiFlag) != 0) toi = c.m_toi;
                else {
                    fA = c.m_fixtureA;
                    fB = c.m_fixtureB;
                    bA = fA.m_body;
                    ...
                    box2D.common.B2Settings.b2Assert(0 <= toi && toi <= 1);
                    if (toi > 0 && toi < 1) {
                        toi = (1 - toi) * t0 + toi;
                        if (toi > 1) toi = 1
                    }
...
box2D.dynamics.contacts.B2Contact.prototype = {
    ...
    setSensor: function (sensor) {
        if (sensor) this.m_flags |= box2D.dynamics.contacts.B2Contact.e_sensorFlag;
        else this.m_flags &= ~box2D.dynamics.contacts.B2Contact.e_sensorFlag
    },
    isSensor: function () {
        return (this.m_flags & box2D.dynamics.contacts.B2Contact.e_sensorFlag) == box2D.dynamics.contacts.B2Contact.e_sensorFlag
    },
    setEnabled: function (flag) {
        if (flag) this.m_flags |= box2D.dynamics.contacts.B2Contact.e_enabledFlag;
        else this.m_flags &= ~box2D.dynamics.contacts.B2Contact.e_enabledFlag
    },
    isEnabled: function () {
    ...
    update: function (listener) {
        ...
        if ((this.m_flags & box2D.dynamics.contacts.B2Contact.e_sensorFlag) != 0) {
            ...
            if (bodyA.getType() != box2D.dynamics.B2Body.b2_dynamicBody || bodyA.isBullet() || bodyB.getType() != box2D.dynamics.B2Body.b2_dynamicBody || bodyB.isBullet()) this.m_flags |= box2D.dynamics.contacts.B2Contact.e_continuousFlag;
            else this.m_flags &= ~box2D.dynamics.contacts.B2Contact.e_continuousFlag; if (aabbOverlap) {
                this.evaluate();
            ...
            } else this.m_manifold.m_pointCount = 0; if (touching != wasTouching) {
                bodyA.setAwake(true);
                bodyB.setAwake(true)
            }
        } if (touching) this.m_flags |= box2D.dynamics.contacts.B2Contact.e_touchingFlag;
        else this.m_flags &= ~box2D.dynamics.contacts.B2Contact.e_touchingFlag; if (wasTouching == false && touching == true) listener.beginContact(this);
...
box2D.dynamics.contacts.B2ContactFactory.prototype = {
    ...
    initializeRegisters: function () {
        ...
        var _g1 = 0;
        var _g = box2D.collision.shapes.B2Shape.e_shapeTypeCount;
        while (_g1 < _g) {
            var i = _g1++;
            this.m_registers[i] = new Array;
            var _g3 = 0;
            var _g2 = box2D.collision.shapes.B2Shape.e_shapeTypeCount;
            while (_g3 < _g2) {
                var j = _g3++;
    ...
    solveVelocityConstraints: function () {
        ...
        while (_g1 < _g) {
            ...
            if (c.pointCount == 1) {
                ...
                if (newImpulse > 0) newImpulse = newImpulse;
                else newImpulse = 0;
    ...
    solvePositionConstraints: function (baumgarte) {
        ...
        while (_g1 < _g) {
            ...
            while (_g3 < _g2) {
                ...
                if (minSeparation < separation) minSeparation = minSeparation;
                else minSeparation = separation;
                var C = box2D.common.math.B2Math.clamp(baumgarte * (separation + box2D.common.B2Settings.b2_linearSlop), -box2D.common.B2Settings.b2_maxLinearCorrection, 0);
...
box2D.dynamics.joints.B2Joint.create = function (def, allocator) {
    var joint = null;
    if (def.type == box2D.dynamics.joints.B2Joint.e_distanceJoint) joint = new box2D.dynamics.joints.B2DistanceJoint(js.Boot.__cast(def, box2D.dynamics.joints.B2DistanceJointDef));
    else if (def.type == box2D.dynamics.joints.B2Joint.e_mouseJoint) joint = new box2D.dynamics.joints.B2MouseJoint(js.Boot.__cast(def, box2D.dynamics.joints.B2MouseJointDef));
    else if (def.type == box2D.dynamics.joints.B2Joint.e_prismaticJoint) joint = new box2D.dynamics.joints.B2PrismaticJoint(js.Boot.__cast(def, box2D.dynamics.joints.B2PrismaticJointDef));
    else if (def.type == box2D.dynamics.joints.B2Joint.e_revoluteJoint) joint = new box2D.dynamics.joints.B2RevoluteJoint(js.Boot.__cast(def, box2D.dynamics.joints.B2RevoluteJointDef));
    else if (def.type == box2D.dynamics.joints.B2Joint.e_pulleyJoint) joint = new box2D.dynamics.joints.B2PulleyJoint(js.Boot.__cast(def, box2D.dynamics.joints.B2PulleyJointDef));
    else if (def.type == box2D.dynamics.joints.B2Joint.e_gearJoint) joint = new box2D.dynamics.joints.B2GearJoint(js.Boot.__cast(def, box2D.dynamics.joints.B2GearJointDef));
    else if (def.type == box2D.dynamics.joints.B2Joint.e_lineJoint) joint = new box2D.dynamics.joints.B2LineJoint(js.Boot.__cast(def, box2D.dynamics.joints.B2LineJointDef));
    else if (def.type == box2D.dynamics.joints.B2Joint.e_weldJoint) joint = new box2D.dynamics.joints.B2WeldJoint(js.Boot.__cast(def, box2D.dynamics.joints.B2WeldJointDef));
    else if (def.type == box2D.dynamics.joints.B2Joint.e_frictionJoint) joint = new box2D.dynamics.joints.B2FrictionJoint(js.Boot.__cast(def, box2D.dynamics.joints.B2FrictionJointDef));
    else {}
    return joint
};
...
box2D.dynamics.joints.B2DistanceJoint.__name__ = true;
...
box2D.dynamics.joints.B2DistanceJoint.prototype = $extend(box2D.dynamics.joints.B2Joint.prototype, {
    ...
    initVelocityConstraints: function (step) {
        ...
        if (length > box2D.common.B2Settings.b2_linearSlop) this.m_u.multiply(1 / length);
        else this.m_u.setZero();
        var cr1u = r1X * this.m_u.y - r1Y * this.m_u.x;
        var cr2u = r2X * this.m_u.y - r2Y * this.m_u.x;
        var invMass = bA.m_invMass + bA.m_invI * cr1u * cr1u + bB.m_invMass + bB.m_invI * cr2u * cr2u;
        if (invMass != 0) this.m_mass = 1 / invMass;
        else this.m_mass = 0; if (this.m_frequencyHz > 0) {
...
box2D.dynamics.joints.B2JointDef.__name__ = true;
...
box2D.dynamics.joints.B2DistanceJointDef.__name__ = true;
...
box2D.dynamics.joints.B2FrictionJoint.__name__ = true;
...
box2D.dynamics.joints.B2FrictionJointDef.__name__ = true;
...
box2D.dynamics.joints.B2GearJoint.__name__ = true;
...
box2D.dynamics.joints.B2GearJoint.prototype = $extend(box2D.dynamics.joints.B2Joint.prototype, {
    ...
    initVelocityConstraints: function (step) {
        ...
        } if (K > 0) this.m_mass = 1 / K;
        else this.m_mass = 0; if (step.warmStarting) {
    ...
    solvePositionConstraints: function (baumgarte) {
        ...
        var coordinate1;
        var coordinate2;
        if (this.m_revolute1 != null) coordinate1 = this.m_revolute1.getJointAngle();
        else coordinate1 = this.m_prismatic1.getJointTranslation(); if (this.m_revolute2 != null) coordinate2 = this.m_revolute2.getJointAngle();
...
box2D.dynamics.joints.B2GearJointDef.__name__ = true;
...
box2D.dynamics.joints.B2Jacobian.__name__ = true;
...
box2D.dynamics.joints.B2LineJoint.__name__ = true;
...
box2D.dynamics.joints.B2LineJoint.prototype = $extend(box2D.dynamics.joints.B2Joint.prototype, {
    ...
    initVelocityConstraints: function (step) {
        ...
        if (this.m_motorMass > 2.2250738585072014e-308) this.m_motorMass = 1 / this.m_motorMass;
        else this.m_motorMass = 0;
        ...
        if (this.m_enableLimit) {
            ...
            if (box2D.common.math.B2Math.abs(this.m_upperTranslation - this.m_lowerTranslation) < 2 * box2D.common.B2Settings.b2_linearSlop) this.m_limitState = box2D.dynamics.joints.B2Joint.e_equalLimits;
        ...
        } else this.m_limitState = box2D.dynamics.joints.B2Joint.e_inactiveLimit; if (this.m_enableMotor == false) this.m_motorImpulse = 0;
        if (step.warmStarting) {
    ...
    solveVelocityConstraints: function (step) {
        ...
        if (this.m_enableLimit && this.m_limitState != box2D.dynamics.joints.B2Joint.e_inactiveLimit) {
            ...
            if (this.m_limitState == box2D.dynamics.joints.B2Joint.e_atLowerLimit) this.m_impulse.y = box2D.common.math.B2Math.max(this.m_impulse.y, 0);
            else if (this.m_limitState == box2D.dynamics.joints.B2Joint.e_atUpperLimit) this.m_impulse.y = box2D.common.math.B2Math.min(this.m_impulse.y, 0);
    ...
    solvePositionConstraints: function (baumgarte) {
        ...
        if (this.m_enableLimit) {
            ...
            if (box2D.common.math.B2Math.abs(this.m_upperTranslation - this.m_lowerTranslation) < 2 * box2D.common.B2Settings.b2_linearSlop) {
                ...
                C2 = box2D.common.math.B2Math.clamp(translation - this.m_lowerTranslation + box2D.common.B2Settings.b2_linearSlop, -box2D.common.B2Settings.b2_maxLinearCorrection, 0);
                linearError = this.m_lowerTranslation - translation;
                active = true
            } else if (translation >= this.m_upperTranslation) {
                C2 = box2D.common.math.B2Math.clamp(translation - this.m_upperTranslation + box2D.common.B2Settings.b2_linearSlop, 0, box2D.common.B2Settings.b2_maxLinearCorrection);
...
box2D.dynamics.joints.B2LineJointDef.__name__ = true;
...
box2D.dynamics.joints.B2MouseJoint.__name__ = true;
...
box2D.dynamics.joints.B2MouseJointDef.__name__ = true;
...
box2D.dynamics.joints.B2PrismaticJoint.__name__ = true;
...
box2D.dynamics.joints.B2PrismaticJoint.prototype = $extend(box2D.dynamics.joints.B2Joint.prototype, {
    ...
    solveVelocityConstraints: function (step) {
        ...
        if (this.m_enableLimit && this.m_limitState != box2D.dynamics.joints.B2Joint.e_inactiveLimit) {
            ...
            this.m_impulse.add(df);
            if (this.m_limitState == box2D.dynamics.joints.B2Joint.e_atLowerLimit) this.m_impulse.z = box2D.common.math.B2Math.max(this.m_impulse.z, 0);
            else if (this.m_limitState == box2D.dynamics.joints.B2Joint.e_atUpperLimit) this.m_impulse.z = box2D.common.math.B2Math.min(this.m_impulse.z, 0);
...
box2D.dynamics.joints.B2PrismaticJointDef.__name__ = true;
...
box2D.dynamics.joints.B2PulleyJoint.prototype = $extend(box2D.dynamics.joints.B2Joint.prototype, {
    ...
    initVelocityConstraints: function (step) {
        ...
        if (length1 > box2D.common.B2Settings.b2_linearSlop) this.m_u1.multiply(1 / length1);
        else this.m_u1.setZero(); if (length2 > box2D.common.B2Settings.b2_linearSlop) this.m_u2.multiply(1 / length2);
        else this.m_u2.setZero();
        var C = this.m_constant - length1 - this.m_ratio * length2;
        if (C > 0) {
            this.m_state = box2D.dynamics.joints.B2Joint.e_inactiveLimit;
            this.m_impulse = 0
        } else this.m_state = box2D.dynamics.joints.B2Joint.e_atUpperLimit; if (length1 < this.m_maxLength1) {
            this.m_limitState1 = box2D.dynamics.joints.B2Joint.e_inactiveLimit;
            this.m_limitImpulse1 = 0
        } else this.m_limitState1 = box2D.dynamics.joints.B2Joint.e_atUpperLimit; if (length2 < this.m_maxLength2) {
    ...
    solveVelocityConstraints: function (step) {
        ...
        if (this.m_state == box2D.dynamics.joints.B2Joint.e_atUpperLimit) {
            ...
            this.m_impulse = box2D.common.math.B2Math.max(0, this.m_impulse + impulse);
        ...
        if (this.m_limitState1 == box2D.dynamics.joints.B2Joint.e_atUpperLimit) {
            ...
            this.m_limitImpulse1 = box2D.common.math.B2Math.max(0, this.m_limitImpulse1 + impulse);
        ...
        if (this.m_limitState2 == box2D.dynamics.joints.B2Joint.e_atUpperLimit) {
            ...
            this.m_limitImpulse2 = box2D.common.math.B2Math.max(0, this.m_limitImpulse2 + impulse);
    ...
    solvePositionConstraints: function (baumgarte) {
        ...
        if (this.m_state == box2D.dynamics.joints.B2Joint.e_atUpperLimit) {
            ...
            C = box2D.common.math.B2Math.clamp(C + box2D.common.B2Settings.b2_linearSlop, -box2D.common.B2Settings.b2_maxLinearCorrection, 0);
...
box2D.dynamics.joints.B2PulleyJointDef.__name__ = true;
...
box2D.dynamics.joints.B2RevoluteJoint.__name__ = true;
...
box2D.dynamics.joints.B2RevoluteJoint.prototype = $extend(box2D.dynamics.joints.B2Joint.prototype, {
    ...
    initVelocityConstraints: function (step) {
        ...
        if (this.m_enableLimit) {
            ...
            if (box2D.common.math.B2Math.abs(this.m_upperAngle - this.m_lowerAngle) < 2 * box2D.common.B2Settings.b2_angularSlop) this.m_limitState = box2D.dynamics.joints.B2Joint.e_equalLimits;
            else if (jointAngle <= this.m_lowerAngle) {
                if (this.m_limitState != box2D.dynamics.joints.B2Joint.e_atLowerLimit) this.m_impulse.z = 0;
                this.m_limitState = box2D.dynamics.joints.B2Joint.e_atLowerLimit
            } else if (jointAngle >= this.m_upperAngle) {
                if (this.m_limitState != box2D.dynamics.joints.B2Joint.e_atUpperLimit) this.m_impulse.z = 0;
                this.m_limitState = box2D.dynamics.joints.B2Joint.e_atUpperLimit
            } else {
                this.m_limitState = box2D.dynamics.joints.B2Joint.e_inactiveLimit;
                this.m_impulse.z = 0
            }
        } else this.m_limitState = box2D.dynamics.joints.B2Joint.e_inactiveLimit; if (step.warmStarting) {
            this.m_impulse.x *= step.dtRatio;
            this.m_impulse.y *= step.dtRatio;
            this.m_motorImpulse *= step.dtRatio;
    ...
    solveVelocityConstraints: function (step) {
        ...
        if (this.m_enableLimit && this.m_limitState != box2D.dynamics.joints.B2Joint.e_inactiveLimit) {
            ...
            if (this.m_limitState == box2D.dynamics.joints.B2Joint.e_equalLimits) this.m_impulse.add(this.impulse3);
    ...
    solvePositionConstraints: function (baumgarte) {
        ...
        if (this.m_enableLimit && this.m_limitState != box2D.dynamics.joints.B2Joint.e_inactiveLimit) {
            ...
            if (this.m_limitState == box2D.dynamics.joints.B2Joint.e_equalLimits) {
                ...
                angularError = -C;
                C = box2D.common.math.B2Math.clamp(C + box2D.common.B2Settings.b2_angularSlop, -box2D.common.B2Settings.b2_maxAngularCorrection, 0);
                limitImpulse = -this.m_motorMass * C
            } else if (this.m_limitState == box2D.dynamics.joints.B2Joint.e_atUpperLimit) {
                C = angle - this.m_upperAngle;
                angularError = C;
                C = box2D.common.math.B2Math.clamp(C - box2D.common.B2Settings.b2_angularSlop, 0, box2D.common.B2Settings.b2_maxAngularCorrection);
                limitImpulse = -this.m_motorMass * C
            }
        ...
        var invI1 = bA.m_invI;
        var invI2 = bB.m_invI;
        var k_allowedStretch = 10 * box2D.common.B2Settings.b2_linearSlop;
...
box2D.dynamics.joints.B2RevoluteJointDef.__name__ = true;
...
box2D.dynamics.joints.B2WeldJoint.__name__ = true;
...
box2D.dynamics.joints.B2WeldJointDef.__name__ = true;
